...
namespace BasicRPGTest_Mono.Engine.Maps
{
	...
	{
		private static Random random = new Random(); //Initializes the random class
		...
		public LevelGen(int w, int h, int featureSize)
		{
			...
			{ // Loops through the width of the map, going up by the featureSize value each time. 
				...
				{ // Loops through the width of the map a second time, going up by the featureSize value each time.
					setSample(x, y, random.NextDouble() * 2 - 1); // sets a random value at a x and y point.
				}
			}
		...
		{
			do
			{ // Keep repeating this loop until it's done...
		...
		private static byte[][] createTopMap(int w, int h)
		{
			...
			{ // Loops through the height of the map
				...
				{ // A loop inside a loop that loops through the width of the map.
					...
					if (val < -0.5)
					{ // If the final value is less than -0.5...
						//map[i] = Tile.water.id; // the tile will become water
					}
					else if (val > 0.5 && mval < -1.5)
					{ // else if the val is larger 0.5 and mval is smaller than -1.5...
						//map[i] = Tile.rock.id; // the tile will become rock
					}
					else
					{ // If the values don't agree with those then...
			...
			{
				int xs = random.Next(w); // A random number between 0 to the map's width (minus 1, because 0 is the first number)
				int ys = random.Next(h); // A random number between 0 to the map's height (minus 1, because 0 is the first number)
				for (int k = 0; k < 10; k++)
				{ // a loop inside the main loop that occurs 10 times.
					int x = xs + random.Next(21) - 10; // x value which is: xs + (random value between 0 to 20) - 10
					int y = ys + random.Next(21) - 10; // y value which is: ys + (random value between 0 to 20) - 10
					for (int j = 0; j < 100; j++)
					{ // A loop inside a loop inside the main loop, repeats 100 times.
						int xo = x + random.Next(5) - random.Next(5); // xo value: x + (random value between 0 to 4) - (random val between 0 to 4)
						int yo = y + random.Next(5) - random.Next(5); // yo value: y + (random value between 0 to 4) - (random val between 0 to 4)
						for (int yy = yo - 1; yy <= yo + 1; yy++) // Loopception, Loops if yy is smaller or equal to yo + 1
							for (int xx = xo - 1; xx <= xo + 1; xx++) // Loopception, Loops if xx is smaller or equal to xo + 1 
								...
								{ // if xx or yy is equal or larger than 0, and smaller than the width and height of the map...
									//if (map[xx + yy * w] == Tile.grass.id)
									{ // If the specific xx and yy coordinates happen to be a grass tile...
			...
			{
				int x = random.Next(w);// A random number between 0 to the map's width (minus 1, because 0 is the first number)
				int y = random.Next(h);// A random number between 0 to the map's height (minus 1, because 0 is the first number)
				for (int j = 0; j < 200; j++)
				{ // A loop that occurs 200 times
					int xx = x + random.Next(15) - random.Next(15); // x + (random value between 0 to 14) - (random value between 0 to 14)
					int yy = y + random.Next(15) - random.Next(15); // y + (random value between 0 to 14) - (random value between 0 to 14)
					if (xx >= 0 && yy >= 0 && xx < w && yy < h)
					{ // if xx or yy is equal or larger than 0, and smaller than the width and height of the map...
						//if (map[xx + yy * w] == Tile.grass.id)
						{ // If the specific xx and yy coordinates happen to be a grass tile...
			...
			for (int i = 0; i < w * h / 400; i++)
			{
				int x = random.Next(w);// A random number between 0 to the map's width (minus 1, because 0 is the first number)
				int y = random.Next(h);// A random number between 0 to the map's height (minus 1, because 0 is the first number)
				int col = random.Next(4); // random number between 0 to 3
				for (int j = 0; j < 30; j++)
				{ // loop that occurs 30 times
					int xx = x + random.Next(5) - random.Next(5); // x + (random value between 0 to 4) - (random value between 0 to 4)
					int yy = y + random.Next(5) - random.Next(5); // y + (random value between 0 to 4) - (random value between 0 to 4)
					if (xx >= 0 && yy >= 0 && xx < w && yy < h)
					{ // if xx or yy is equal or larger than 0, and smaller than the width and height of the map...
						//if (map[xx + yy * w] == Tile.grass.id)
						{ // If the specific xx and yy coordinates happen to be a grass tile...
							//map[xx + yy * w] = Tile.flower.id; // replace the tile with a flower tile
							//data[xx + yy * w] = (byte)(col + random.Next(4) * 16); // Adds data to the tile, (flipping it sideways)
						}
					}
				}
			}
			...
			{
				int xx = random.Next(w);// A random number between 0 to the map's width (minus 1, because 0 is the first number)
				int yy = random.Next(h);// A random number between 0 to the map's height (minus 1, because 0 is the first number)
				if (xx >= 0 && yy >= 0 && xx < w && yy < h)
				{ // if xx or yy is equal or larger than 0, and smaller than the width and height of the map...
					//if (map[xx + yy * w] == Tile.sand.id)
					{ // If the specific xx and yy coordinates happen to be a sand tile...
		...
		stairsLoop: for (int i = 0; i < w * h / 100; i++)
			{
				int x = random.Next(w - 2) + 1; // A random number between 0 to the map's width minus 2, plus one.
				int y = random.Next(h - 2) + 1; // A random number between 0 to the map's width minus 2, plus one.
		...
		private static byte[][] createUndergroundMap(int w, int h, int depth)
		{
			...
			{ // Loops through the height of the map
				...
				{ // Loops through the width of the map
					...
					{//if val is larger than -2, and mval is smaller than -1.7 OR nval is smaller than -1.4 then...
...

			{
				...
				for (int i = 0; i < w * h / 400; i++)
				{
					...
					{ // A loop that occurs 30 times
						int xx = x + random.Next(5) - random.Next(5); // x + (random number between 0 to 4) - (random number between 0 to 4)
						int yy = y + random.Next(5) - random.Next(5); // y + (random number between 0 to 4) - (random number between 0 to 4)
						if (xx >= r && yy >= r && xx < w - r && yy < h - r)
						{ // If xx & yy are equal to or larger than r, and smaller than (w - r) and (h - r) then...
							//if (map[xx + yy * w] == Tile.rock.id)
							{ // If the current tile is a rock tile...
			...
			if (depth < 3)
			{ // If the depth is smaller than 3 then...
				int count = 0; // count of stairs
			stairsLoop: for (int i = 0; i < w * h / 100; i++)
				{
					int x = random.Next(w - 20) + 10; // A random number between 0 to the map's width minus 20, plus 10.
					int y = random.Next(h - 20) + 10; // A random number between 0 to the map's width minus 20, plus 10.
		...
		private static byte[][] createSkyMap(int w, int h)
		{
			...
			{ // loops through the height of the map
				...
				{ // loops though the width of the map
					...
					if (val < -0.25)
					{ // If val is smaller than -0.25 then...
		...
		cactusLoop: for (int i = 0; i < w * h / 50; i++)
